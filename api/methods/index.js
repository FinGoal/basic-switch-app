...
export const getUserToken = async (req, res, next) => {
  const { body } = req;
  const { userId } = body;
  const organization = body.organization ? body.organization : "switchkit";
  ...
  if (!userId) {
    ...
    const data = {
      ...
      organization, // a test organization name for this demo. Do not alter this - doing so will interfere with your token's permissions and ability to load the proper Yodlee Fastlink OB configuration.
      userId,
    };

    try {
      const response = await axios.post(
        process.env.LINK_MONEY_AUTHENTICATION_URL,
        data
      );
